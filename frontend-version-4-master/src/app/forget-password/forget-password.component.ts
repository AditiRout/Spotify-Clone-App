...
import { User } from '../user.model'; // Import the Forget model
...
export class ForgetPasswordComponent {
  ...
  onSubmit(): void {
    if (this.forgotPasswordForm.valid) {
      ...
      this.userService.resetPassword(this.user.email,this.user.securityQuestion, this.user.securityAnswer, this.user.password).subscribe(
        (response) => {
        ...
        (error) => {
          ...
          if (error.status === 400 || error.status === 401) {  // Assuming 400 or 401 indicates invalid details
            ...
            this.openSnackBar('Invalid details. Please try again', 'Close');  // Generic error message
          }
        }
      );
    } else {
